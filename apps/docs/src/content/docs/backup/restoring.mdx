...
- **Permissions.** Restored files retain the permissions from the snapshot. On Linux and macOS, Unix mode bits and the original modification time (mtime) are reapplied to each restored file. If restoring to a different device or OS, permissions may need adjustment.
- **System-image restores.** Windows system-state (system image) snapshots restore byte-for-byte — the restored artifacts match the original backup exactly.
